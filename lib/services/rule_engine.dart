...
class RuleEngine {
  ...
  // ═══════════════════════════════════════════════════════════
  // EVALUATE ALL RULES AGAINST ALL CAMPAIGNS
  // ═══════════════════════════════════════════════════════════
  ...
  ActionDetails _buildAction(AutoRule rule, Campaign campaign) {
    return switch (rule.actionType) {
      'pause' => ActionDetails(
        type: 'pause',
        description: 'Pause "${campaign.name}" — ${rule.metric} ${rule.operator} ${rule.threshold}',
        newStatus: 'PAUSED',
      ),
      'scale_budget' => ActionDetails(
        type: 'scale_budget',
        description: 'Scale "${campaign.name}" budget +${rule.actionValue?.toInt()}%',
        newBudget: campaign.dailyBudget * (1 + (rule.actionValue ?? 20) / 100),
      ),
      'reduce_budget' => ActionDetails(
        type: 'reduce_budget',
        description: 'Reduce "${campaign.name}" budget -${rule.actionValue?.toInt()}%',
        newBudget: campaign.dailyBudget * (1 - (rule.actionValue ?? 30) / 100),
      ),
      'alert' => ActionDetails(
        type: 'alert',
        description: 'Alert: "${campaign.name}" ${rule.metric} is ${_getMetricValue(rule.metric, campaign)?.toStringAsFixed(1)}',
      ),
      'alert_and_pause' => ActionDetails(
        type: 'alert_and_pause',
        description: 'Alert + Pause: "${campaign.name}" — ${rule.metric} exceeded threshold',
        newStatus: 'PAUSED',
      ),
      _ => ActionDetails(type: 'unknown', description: 'Unknown action'),
    };
  }
  ...
  Future<RuleResult> _executeAction(RuleAction action) async {
    // In production, this would call MetaApiService
